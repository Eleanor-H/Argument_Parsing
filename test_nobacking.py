...
#        not_relevant_Backing_coor = set([tuple(item) for item in torch.nonzero(not_relevant_Backing)])
        ...
        tp_NTC = len(selected_NTC_coor & relevant_NTC_coor) #+ 1
        tp_Claim = len(selected_Claim_coor & relevant_Claim_coor) 
        tp_Datum = len(selected_Datum_coor & relevant_Datum_coor)# + 3
        tp_Warrant = len(selected_Warrant_coor & relevant_Warrant_coor)
#        tp_Backing = len(selected_Backing_coor & relevant_Backing_coor) + 1
        tp_all = tp_NTC + tp_Claim + tp_Datum + tp_Warrant #+ tn_Backing
...
#        precision_Backing = tp_Backing / (len(selected_Backing_coor)+1)
...
#        F1_Backing = 2 * precision_Backing * recall_Backing / (precision_Backing + recall_Backing)

        macro_avg = (F1_NTC + F1_Claim + F1_Datum + F1_Warrant) / 4
...
#        print('Backing:        %d           %d             %d              %.4f          %.4f          %.4f' 
#            % (tp_Backing, torch.sum(selected_Backing), torch.sum(relevant_Backing), precision_Backing, recall_Backing, F1_Backing))
        print('-----------------------------------------------------------------------------------------------')
        print('-----------------------------------------------------------------------------------------------')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_file', type=str, default='./corpus/val_nobacking.json')
